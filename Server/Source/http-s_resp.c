                 ...
                 case HTTPs_ERR_NONE:
                      ...
                      if (body_present == DEF_NO) {
                          ...
                          if (chunk_hook_en == DEF_YES) {
                              p_conn->State = HTTPs_CONN_STATE_RESP_DATA_CHUNKED_HOOK;
                          } else {
                              if (chunk_en == DEF_YES) {
                                  p_conn->State = HTTPs_CONN_STATE_RESP_DATA_CHUNKED;
                              } else {
    ...
    switch (p_conn->State) {
        case HTTPs_CONN_STATE_RESP_DATA_CHUNKED_HOOK:
             ...
             if (is_last_chunk == DEF_TRUE) {                   /* ... if it is the last chunk ...                      */
                 ...
                 p_conn->State = HTTPs_CONN_STATE_RESP_DATA_CHUNKED_FINALIZE;
        ...
        default:
             HTTPs_ERR_INC(p_ctr_err->Resp_ErrTransferChunkedHookStateInvCtr);
            *p_err = HTTPs_ERR_STATE_UNKNOWN;
             goto exit;
    }
                                                    ...
                                                    HTTPs_ERR       *p_err)
{
    ...
    switch (p_conn->State) {
        case HTTPs_CONN_STATE_RESP_DATA_CHUNKED:
             ...
             if ((tx_buf   == DEF_YES) &&                       /* Tx only file data chunk.                             */
                 ...
                 p_conn->State = HTTPs_CONN_STATE_RESP_DATA_CHUNKED;
                        ...
                        (tx_token == DEF_YES)) {
                 p_conn->State     = HTTPs_CONN_STATE_RESP_DATA_CHUNKED_TX_TOKEN;
                 ...
                 p_conn->State = HTTPs_CONN_STATE_RESP_DATA_CHUNKED_TX_LAST_CHUNK;
        ...
        default:                                                /* Inv state.                                           */
             HTTPs_ERR_INC(p_ctr_err->Resp_ErrTransferChunkedStateInvCtr);
            *p_err = HTTPs_ERR_STATE_UNKNOWN;
             goto exit;
    }
                                        ...
                                        HTTPs_CONN      *p_conn)
{
    ...
    p_conn->DataLen          = HTTPs_HTML_DFLT_ERR_LEN;
