...
#define  TCPIP_PAGE_URL             "/tcpip.html"
...
*
*          (4)  If the upper application want to parse the data received in the request body, a hook function is available.
*               It will be called each time new data are received. The exception is when a POST request with a form is
*               received. In that case, the HTTP server core will parse the body and saved the data into Key-Value data blocks.
*               If the hook is not required by the upper application, it can be set as DEF_NULL and no function will be called.
...
const  HTTPs_HOOK_CFG  HTTPs_Hooks = {
                                    ...
                                    const  void            *p_hook_cfg)
{
    ...
    HTTPs_ERR   err;
                                                                ...
                                                                /* Redirect the TCPIP page example to the default page. */
    str_cmp = Str_Cmp_N(p_conn->PathPtr, TCPIP_PAGE_URL, p_conn->PathLenMax);
    if (str_cmp == 0) {
...
*               DEF_NO,     otherwise.
*
* Caller(s)   : HTTPs_RespPrepareHdr() via 'p_cfg->OnRespHdrTxHook()'.
*
           ...
           case HTTP_STATUS_OK:
                ...
                if (p_conn->ReqContentType == HTTP_CONTENT_TYPE_HTML) {
                    ...
                    p_resp_hdr_blk = HTTPs_RespHdrGet((HTTPs_INSTANCE *)p_instance,
                                                                       ...
                                                                       &http_err);
                    if (p_resp_hdr_blk == (HTTP_HDR_BLK *)0) {
                        return(DEF_FAIL);
                    }
...
*                                   HTTPs_FILE_TYPE_FILE,         when file is include in a File System.
                                           ...
                                           CPU_INT32U             *p_data_len)
{
    switch (status_code) {
        case HTTP_STATUS_NOT_FOUND:
             Str_Copy_N(p_file_str, HTTPs_CFG_INSTANCE_SECURE_STR_FILE_ERR_404, file_len_max);
